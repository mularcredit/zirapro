...
const useStatutorySettings = () => {
  ...
  const migrateSettings = (savedSettings: any) => {
  ...
  const calculatePAYE = (taxableIncome: number, _hasTaxPIN?: boolean): number => {
  ...
  const calculateHousingLevy = (grossSalary: number, hasTaxPIN: boolean): number => {

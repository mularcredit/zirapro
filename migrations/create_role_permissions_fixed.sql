-- CORRECTED VERSION: Role & Permissions Migration Script
-- This version handles case-sensitive column names properly
...
CREATE POLICY "Only admins can modify permissions" ON permissions
  FOR ALL USING (
    EXISTS (
      ...
      WHERE "Work Email" = (SELECT auth.jwt() ->> 'email')
      AND "Role" = 'ADMIN'
    )
  );
...
BEGIN
  -- Get user's role (using quoted column name for case sensitivity)
